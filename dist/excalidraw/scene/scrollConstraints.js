 ...
 */
export const calculateConstrainedScrollCenter = (state, { scrollX, scrollY }) => {
    const { width, height, zoom, scrollConstraints } = state;
    if (!scrollConstraints) {
        return { scrollX, scrollY, zoom };
    }
    const { zoomLevelX, zoomLevelY, initialZoomLevel } = calculateZoomLevel(scrollConstraints, width, height);
    // The zoom level to contain the whole constrained area in view
    const _zoom = {
        value: getNormalizedZoom(initialZoomLevel ?? Math.min(zoomLevelX, zoomLevelY)),
    };
 ...
 */
const calculateZoomLevel = (scrollConstraints, width, height) => {
    ...
    const initialZoomLevel = getNormalizedZoom(Math.min(zoomLevelX, zoomLevelY) * viewportZoomFactor);
    return { zoomLevelX, zoomLevelY, initialZoomLevel };
};
const calculateConstraints = ({ scrollConstraints, width, height, zoom, allowOverscroll, }) => {
     ...
     */
    const calculateConstrainedScrollCenter = (zoom) => {
        const constrainedScrollCenterX = scrollConstraints.x + (scrollConstraints.width - width / zoom) / -2;
        const constrainedScrollCenterY = scrollConstraints.y + (scrollConstraints.height - height / zoom) / -2;
     ...
     */
    const calculateMinMaxScrollValues = (shouldAdjustForCenteredViewX, shouldAdjustForCenteredViewY, overscrollAllowance, constrainedScrollCenterX, constrainedScrollCenterY, zoom) => {
        let maxScrollX;
        let minScrollX;
        let maxScrollY;
        let minScrollY;
    ...
    const { zoomLevelX, zoomLevelY, initialZoomLevel } = calculateZoomLevel(scrollConstraints, width, height);
    const constrainedZoom = getNormalizedZoom(scrollConstraints.lockZoom
        ? Math.max(initialZoomLevel, zoom.value)
        : zoom.value);
    const { constrainedScrollCenterX, constrainedScrollCenterY } = calculateConstrainedScrollCenter(constrainedZoom);
    ...
    const { maxScrollX, minScrollX, maxScrollY, minScrollY } = calculateMinMaxScrollValues(shouldAdjustForCenteredViewX, shouldAdjustForCenteredViewY, overscrollAllowance, constrainedScrollCenterX, constrainedScrollCenterY, constrainedZoom);
    return {
        maxScrollX,
        minScrollX,
        maxScrollY,
 ...
 */
const constrainScrollValues = ({ scrollX, scrollY, maxScrollX, minScrollX, maxScrollY, minScrollY, constrainedZoom, }) => {
    const constrainedScrollX = Math.min(maxScrollX, Math.max(scrollX, minScrollX));
    const constrainedScrollY = Math.min(maxScrollY, Math.max(scrollY, minScrollY));
    return {
 ...
 */
const isViewportOutsideOfConstrainedArea = (state) => {
    ...
    return (adjustedScrollX > scrollConstraints.x ||
 ...
 */
export const constrainScrollState = (state, allowOverscroll = true) => {
    if (!state.scrollConstraints) {
        return state;
    }
    const { scrollX, scrollY, width, height, scrollConstraints, zoom } = state;
    const canUseMemoizedValues = memoizedValues && // there are memoized values
        ...
        isShallowEqual(state.scrollConstraints, memoizedValues.previousState.scrollConstraints) &&
        // current zoom and window dimensions are equal to those in memoizedValues
        isShallowEqual({ zoom: zoom.value, width, height }, {
